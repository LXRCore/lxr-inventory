...
AddStateBagChangeHandler('isLoggedIn', ('player:%s'):format(sid), function(_, _, value)
    ...
    PlayerData = value and exports['lxr-core']:GetPlayerData() or {}
    isLoggedIn = value
end)
...
RegisterNetEvent('lxr-inventory:client:UpdateItems', function(slot, data)
...
RegisterNetEvent('inventory:client:CheckOpenState', function(type, id, label)
    local name = exports['lxr-core']:SplitStr(label, "-")[2]
    if type == "stash" then
...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        ...
        SendNUIMessage({
            ...
            other = other,
            maxweight = exports['lxr-core']:GetConfig().Player.MaxWeight,
            Ammo = PlayerAmmo,
...
RegisterNetEvent('lxr-inventory:client:giveAnim', function()
...
RegisterNUICallback('Notify', function(data)
    exports['lxr-core']:Notify(9, data.message, data.type)
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    exports['lxr-core']:Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        StopAnimTask(ped, aDict, aLib, 1.0)
        exports['lxr-core']:Notify(9, Lang:t("error.failed"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end)
end)
...
RegisterNUICallback("GiveItem", function(data)
    local ped = PlayerPedId()
    local player, distance = exports['lxr-core']:GetClosestPlayer(GetEntityCoords(ped))
    if player ~= -1 and distance < 3 then
        if (data.inventory == 'player') then
            ...
            exports['lxr-core']:Notify(9, Lang:t("error.not_owned"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
        end
    else
        exports['lxr-core']:Notify(9, Lang:t("error.no_near"), 5000, 0, 'mp_lobby_textures', 'cross', 'COLOR_WHITE')
    end
end)
